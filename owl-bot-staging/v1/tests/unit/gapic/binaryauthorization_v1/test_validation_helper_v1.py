...
from google.cloud.binaryauthorization_v1.services.validation_helper_v1 import ValidationHelperV1AsyncClient
...
from google.cloud.binaryauthorization_v1.services.validation_helper_v1.transports.base import _GOOGLE_AUTH_VERSION
...
def test_validation_helper_v1_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_validate_attestation_occurrence(transport: str = 'grpc', request_type=service.ValidateAttestationOccurrenceRequest):
    client = ValidationHelperV1Client(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
    ...
    with mock.patch.object(
            type(client.transport.validate_attestation_occurrence),
            '__call__') as call:
...
# removed from grpc/grpc_asyncio transport constructor.
@pytest.mark.parametrize("transport_class", [transports.ValidationHelperV1GrpcTransport, transports.ValidationHelperV1GrpcAsyncIOTransport])
def test_validation_helper_v1_transport_channel_mtls_with_adc(
    transport_class
):
    mock_ssl_cred = mock.Mock()
