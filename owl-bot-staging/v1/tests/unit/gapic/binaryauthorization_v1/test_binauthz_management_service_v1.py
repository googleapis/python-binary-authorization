...
from google.cloud.binaryauthorization_v1.services.binauthz_management_service_v1.transports.base import _GOOGLE_AUTH_VERSION
...
def test_binauthz_management_service_v1_client_client_options_credentials_file(client_class, transport_class, transport_name):
...
def test_create_attestor(transport: str = 'grpc', request_type=service.CreateAttestorRequest):
    ...
    with mock.patch.object(
        ...
        call.return_value = resources.Attestor(
            name='name_value',
            description='description_value',
            user_owned_grafeas_note=resources.UserOwnedGrafeasNote(note_reference='note_reference_value'),
        )
...
@requires_google_auth_lt_1_25_0
def test_binauthz_management_service_v1_transport_auth_adc_old_google_auth(transport_class):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
    with mock.patch.object(google.auth, "default", autospec=True) as adc:
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
